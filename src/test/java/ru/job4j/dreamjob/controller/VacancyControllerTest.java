...
public class VacancyControllerTest {
    ...
    @Test
    public void whenFindVacancyById() {
        var vacancy = new Vacancy(1, "test", "desc", now(), true, 1, 2);
        var expectedId = vacancy.getId();
        when(vacancyService.findById(expectedId)).thenReturn(Optional.of(vacancy));
        var model = new ConcurrentModel();

        String viewName = vacancyController.getById(model, expectedId);

        assertThat(viewName).isEqualTo("vacancies/one");
        assertThat(model.containsAttribute("vacancy")).isTrue();
        assertThat(model.getAttribute("vacancy")).isEqualTo(vacancy);
    }

    @Test
    public void whenCantFindVacancyById() throws Exception {
        var notExistId = 10;
        when(vacancyService.findById(notExistId)).thenReturn(Optional.empty());
        var model = new ConcurrentModel();

        String viewName = vacancyController.getById(model, notExistId);
    ...
    public void whenDeleteById() {
        int nonExistId = 10;
        when(vacancyService.deleteById(nonExistId)).thenReturn(false);
        var model = new ConcurrentModel();
        String viewName = vacancyController.delete(model, nonExistId);
        assertThat(viewName).isEqualTo("errors/404");
        assertThat(model.getAttribute("message")).isEqualTo("Вакансия с указанным идентификатором не найдена");
    }

    @Test
    public void whenUpdateVacancyAndGetErrorPage() throws Exception {
        var vacancy = new Vacancy(1, "test", "desc", now(), true, 1, 2);
    ...
    @Test
    public void whenUpdateNonExistentVacancyThenGetError() throws Exception {
        var vacancy = new Vacancy(1, "test", "desc", now(), true, 1, 2);
