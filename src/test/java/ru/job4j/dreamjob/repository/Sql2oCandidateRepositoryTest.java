...
class Sql2oCandidateRepositoryTest {
    ...
    public void whenSaveThenGetSame() {
        ...
        var candidate = sql2oCandidateRepository.save(new Candidate(0, "name", "title", creationDate, 1, file.getId()));
        var savedCandidate = sql2oCandidateRepository.findById(candidate.getId()).get();
        var expectedDate = candidate.getCreationDate().truncatedTo(ChronoUnit.MINUTES);
        var actualDate = savedCandidate.getCreationDate().truncatedTo(ChronoUnit.MINUTES);
    ...
    public void whenSaveSeveralThenGetAll() {
        var creationDate = now().truncatedTo(ChronoUnit.MINUTES);
        var candidate1 = sql2oCandidateRepository.save(new Candidate(0, "name1", "title1", creationDate, 1, file.getId()));
        var candidate2 = sql2oCandidateRepository.save(new Candidate(0, "name2", "title2", creationDate, 1, file.getId()));
        var candidate3 = sql2oCandidateRepository.save(new Candidate(0, "name3", "title3", creationDate, 1, file.getId()));
    ...
    public void whenUpdateThenGetUpdated() {
        ...
        var updatedCandidate = new Candidate(
                candidate.getId(), "new name", "new title", creationDate, 1, file.getId()
        );
        ...
        var expectedDate = updatedCandidate.getCreationDate().truncatedTo(ChronoUnit.MINUTES);
        var actualDate = savedCandidate.getCreationDate().truncatedTo(ChronoUnit.MINUTES);
        assertThat(actualDate).isEqualTo(expectedDate);
        assertThat(savedCandidate)
                .usingRecursiveComparison()
    ...
    public void whenUpdateUnExistingVacancyThenGetFalse() {
        var creationDate = now().truncatedTo(ChronoUnit.MINUTES);
        var candidate = new Candidate(0, "name", "title", creationDate, 1, file.getId());
